...
bool CMyPhysicsView::bshow_force = false;
bool CMyPhysicsView::bshowCube = false;
bool CMyPhysicsView::bshowSphere = false;
bool CMyPhysicsView::bshowothers = false;
...
void CMyPhysicsView::OnSize(UINT nType, int cx, int cy)
{
	...
	renderer_width = 435 + (now_rect.right-now_rect.left)-(wnd_rect.right-wnd_rect.left);
...
{
	//if(view_h>20)posturn=false;
	//if(view_h<-20) posturn=true;
	//if(posturn) view_h+=0.1;
	//else view_h-=0.1;
	//Draw();
	if(nIDEvent = 0) KillTimer(1);
